...
app.controller('AppCtrl', function($rootScope, $scope, ApiService, $http, ConfigurationParams, $uibModal){
  var self = this;
...
//				 ConfigurationParams.get("client_secret"),
  ...
  self.cloneSensor = function(sensor){
	  ...
	  self.sensorList.push(clonedSensor);
	  self.sensorSelected = clonedSensor;
	  self.sensorSelectedTempCopy = angular.copy(clonedSensor);
  };
  ...
  self.getSensorDataList = function(sensorList){
  ...
  self.toggleSensorsSelection = function toggleSelection(fruitName) {
	    ...
	    self.getSensorDataList($scope.selection);
